...
function M.display_inline_diff(buffer, hunks)
  ...
  for _, hunk in ipairs(hunks) do
    ...
    -- Debug hunk info
    local hunk_debug = string.format("Hunk: old_start=%d, old_count=%d, new_start=%d, new_count=%d", 
    ...
    for _, line in ipairs(hunk.lines) do
      ...
      elseif first_char == '+' then
        ...
        local mark_id = vim.api.nvim_buf_set_extmark(buffer, ns_id, line_idx, 0, {
          virt_text = {{M.config.line_symbols.add .. " " .. line_text, hl_group}},
...
function M.get_git_file_content(file_path)
  local relative_path = vim.fn.system(string.format(
...
function M.show_git_diff()
  ...
  -- Debug
  vim.api.nvim_echo({{"Current content length: " .. #current_content .. ", Git content length: " .. #git_content, "Normal"}}, false, {})
...
-- Show diff of the current buffer compared to its saved state
function M.show_buffer_diff()
  ...
  if vim.fn.filereadable(file_path) == 0 then
    vim.api.nvim_echo({{"File doesn't exist on disk", "ErrorMsg"}}, false, {})
    return false
  end
  
  local saved_content = vim.fn.readfile(file_path)
  saved_content = table.concat(saved_content, "\n")
  
  -- Check if there are any changes at all
  if current_content == saved_content then
    vim.api.nvim_echo({{"No changes detected between buffer and file", "WarningMsg"}}, false, {})
    return false
  end
  ...
  vim.fn.writefile(vim.split(current_content, "\n"), temp_current)
  vim.fn.writefile(vim.split(saved_content, "\n"), temp_saved)
  
  -- Debug
  vim.api.nvim_echo({{"Current content length: " .. #current_content .. ", Saved content length: " .. #saved_content, "Normal"}}, false, {})
  
  -- Get diff output
  local diff_cmd = string.format("diff -u %s %s", temp_saved, temp_current)
  local diff_output = vim.fn.system(diff_cmd)
...
-- Show diff based on the configured mode
function M.show_diff()
  if M.config.default_diff_mode == "git" then
