...
# Choose between legacy and new test runners
if [ "$1" == "new" ]; then
  echo "Running new modular tests for unified.nvim..."
  echo "Plugin directory: $PLUGIN_DIR"
  echo "Results will be written to: $RESULTS_FILE"

  # Run Neovim with minimal config, loading the plugin and running the new test runner
  nvim --headless \
    -u NONE \
    -c "set rtp+=$PLUGIN_DIR" \
    -c "source $PLUGIN_DIR/plugin/unified.lua" \
    -c "lua require('test.test_runner')" \
    || { echo "Tests FAILED!"; exit 1; }
else
  echo "Running legacy tests for unified.nvim..."
  echo "Plugin directory: $PLUGIN_DIR"
  echo "Results will be written to: $RESULTS_FILE"
